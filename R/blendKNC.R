...
#return the furthest point from index (that is in the same cluster)
furthest_point <- function(distMatrix, index, labels) {
    cluster_num <- labels[[index]]
    distances <- distMatrix[index, ]
    len <- length(distances)
    maxindex <- 0
    maxlen <- -1
    for(i in 1:len) {
        if(labels[[i]] != cluster_num) next;
        if(distances[i] > maxlen) {
            maxlen <- distances[i]
            maxindex <- i
        }
    }
...
size_of_cluster <- function(cluster_number, labels) {
    size <- 0
...
nth_dataset_in_cluster <- function(cluster_number, n, labels) {
    s <- 0
    for(i in 1:length(labels)) {
        if(labels[[i]] == cluster_number) {
            s <- s+1
...
kmeans_clust <- function(datasets, distMat, k, num_iters) {
    ...
    dim <- ncol(datasets)
    centroids <- sample.int(npoints, k) # in nc kmeans, cluster centroid MUST always be one of the datapoints, so can store just indices
...
#    centroids <- matrix(as.vector(centroids), nrow=k, ncol=dim, byrow=TRUE)
    ...
    labels <- update_labels(distMat, centroids, labels)
    j <- 0
    converged <- FALSE
    while(j < num_iters && converged == FALSE) {
        ...
        for(i in 1:k) {
            tmp <- centroids[i]
            centroids[i] <- update_centroid(distMat, i, labels)
#            tmp <- centroids[i, ]
#            centroids[i, ] <- update_centroid(distMat, i, labels, tmp)
