...
std::vector<int> read_command_line(int argc,char** argv)
{
 // Interpret command line, decides on Modes and throw exceptions
 int runmode;
 std::vector<int> crystals_choice;
 ...
 {
  // Possible Mode 1, Mode 2 or Mode 3. Further checks.
  if (argc == 2)
  {
   // Possible Mode 1. Further checks.
   if (std::atoi(argv[1]) == 0)
   {
    // OK. Mode 1. No further checks needed here.
    runmode=1;
   }
   else
   {
  ...
  {
   // Possible Mode 2 or Mode 3. Further checks.
   std::string tmpstring=argv[2];
   if (tmpstring == "HKLOUT")
   {
    // Possible Mode 3. Further checks needed.
    if (argc > 4)
    {
     // Possible Mode 3. Further checks needed.
     if (std::atoi(argv[3]) == 0)
     {
      // Possible Mode 3. Further checks needed.
      int tmpflag=0;
      for (int i=4;i < argc;i++) if (std::atoi(argv[i]) == 0) tmpflag=1;
      if (tmpflag == 0)
      {
       // OK. Mode 3. No further checks needed here.
       runmode=3;
       for (int i=4;i < argc;i++) crystals_choice.push_back(std::atoi(argv[i])-1);
      }
      else
      {
       // Error coded as 5
   ...
   {
    // Possible Mode 2. Further checks needed.
    int tmpflag=0;
    for (int i=2;i < argc;i++) if (std::atoi(argv[i]) == 0) tmpflag=1;
    if (std::atoi(argv[1]) == 0 & tmpflag == 0)
    {
     // OK. Mode 2. No further checks needed here.
     runmode=2;
     for (int i=2;i < argc;i++) crystals_choice.push_back(std::atoi(argv[i])-1);
    }
    else
    {
     if (std::atoi(argv[1]) != 0)
 ...
 crystals_choice.push_back(runmode);

 return crystals_choice;
}
...
std::vector<scala::hkl_unmerge_list> load_crystals(std::string filename,int runmode)
{
 ...
 else
 {
  ...
  while (!file.eof())
  {
   ...
   {
    // Check this is really an mtz file
 ...
 std::cout << std::endl;
 std::cout << "Loading datasets from all crystals ........." << std::endl;
 ...
 if (runmode == 1)
 {
  ...
  new_file.close();
  std::cout << "Final list of MTZ files which have been processed is included in file " << new_filename << "." << std::endl;
 }
...
std::vector<int> label_crystals(const std::vector<scala::hkl_unmerge_list>& hkl_list,std::vector<int> crystal_flag_in)
{
 ...
 for (int i=0;i < hkl_list.size();i++)
 {
  ...
  std::cout << "Dataset " << i << " number of runs " << hkl_list[i].num_runs() << std::endl;
  std::cout << "Dataset " << i << " number of datasets " << hkl_list[i].num_datasets() << std::endl;
 ...
 for (int i=0;i < spacegroup_class.size();i++)
 {
  ...
  for (pos_cs=pos_pair.first;pos_cs != pos_pair.second;++pos_cs)
  {
   ...
   if (crystal_flag[pos_cs->second] == 0)
   {
    ...
    summary_file << "  " << std::setw(10) << pos_cs->second+1 << "  |" << std::setw(10) << std::setprecision(3) << unitcell.VCell()[0]
                                                              << std::setw(10) << std::setprecision(3) << unitcell.VCell()[1]
                                                              << std::setw(10) << std::setprecision(3) << unitcell.VCell()[2]
                                                              << std::setw(9) << std::setprecision(2) << unitcell.VCell()[3]
                                                              << std::setw(9) << std::setprecision(2) << unitcell.VCell()[4]
                                                              << std::setw(9) << std::setprecision(2) << unitcell.VCell()[5] << "    |"
  ...
  if (spacegroup_class[i] != 0)
  {
   ...
   for (pos_cs=pos_pair.first;pos_cs != pos_pair.second;++pos_cs)
   {
    if (crystal_flag[pos_cs->second] == 0)
    {
     ...
     forR_file2 << "  " << std::setw(10) << pos_cs->second+1 << std::setw(10) << std::setprecision(3) << unitcell.VCell()[0]
                                                             << std::setw(10) << std::setprecision(3) << unitcell.VCell()[1]
                                                             << std::setw(10) << std::setprecision(3) << unitcell.VCell()[2]
                                                             << std::setw(9) << std::setprecision(2) << unitcell.VCell()[3]
                                                             << std::setw(9) << std::setprecision(2) << unitcell.VCell()[4]
